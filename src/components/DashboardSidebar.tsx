...
const DashboardSidebar = ({ setSelectedSidebar }: Iprops) => {
  ...
  return (
    <div className="w-[16%] hidden sm:block min-w-[220px] min-h-screen px-6 py-6 bg-[#14142B]">
      <img className="cursor-pointer" src={logo} alt="logo" />
      <div className="flex flex-col h-[92%] items-center justify-between">
        <div className="mt-[10vh] flex-1">
          {SIDEBAR_NAVIGATIONS.map((d, i) => (
            <div
              key={i.toString()}
              onClick={() => {
                setSelected(i.toString());
                setSelectedSidebar(d.title);
              }}
              className={`mb-6 flex items-center space-x-3 cursor-pointer`}
            >
              <img
